...
const LOT = {
  stages: ["extract", "classify"], // el backend reordena a STAGE_ORDER: comparar como conjunto
...
describe("runRequestMatchesLot · divergencia form ↔ lote congelado", () => {
  ...
  it("filtros ausentes en el eco del lote cuentan como null", () => {
    expect(runRequestMatchesLot(req(), { stages: ["classify", "extract"], filters: {}, force: false })).toBe(
      true,
    )
  })

...
def test_config_loads_from_toml(local_home: Path) -> None:
    ...
    (local_home / "config.toml").write_text(
        'bridge_url = "http://memex.example/"\napi_token = "tok-1"\n',
        encoding="utf-8",
    )
    cfg = LocalConfig.load()
    assert cfg.bridge_url == "http://memex.example/"
...
def test_env_overrides_file(local_home: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    paths.ensure_layout()
    (local_home / "config.toml").write_text(
        'bridge_url = "http://from-file/"\napi_token = "tok-file"\n',
        encoding="utf-8",
    )
    monkeypatch.setenv("MEMEX_LOCAL_BRIDGE_URL", "http://from-env/")
    monkeypatch.setenv("MEMEX_LOCAL_TOKEN", "tok-env")
    cfg = LocalConfig.load()
    assert cfg.bridge_url == "http://from-env/"
    assert cfg.api_token == "tok-env"


def test_missing_bridge_url_raises(local_home: Path) -> None:
    paths.ensure_layout()

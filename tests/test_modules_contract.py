...
def test_validate_item_valid_in_lote() -> None:
    item = validate_item(TransactionItem, dict(_VALID), lote=frozenset({1}))
    assert isinstance(item, TransactionItem)
    assert str(item.amount) == "4500.00"
    assert item.source_inbox_ids == (1,)


def test_validate_item_id_outside_lote_discarded() -> None:
    assert validate_item(TransactionItem, dict(_VALID), lote=frozenset({2, 3})) is None


def test_validate_item_empty_attribution_discarded() -> None:
    raw = {**_VALID, "source_inbox_ids": []}
    assert validate_item(TransactionItem, raw, lote=frozenset({1})) is None
...
def test_validate_item_evidence_miss_keeps_item() -> None:
    ...
    item = validate_item(TransactionItem, raw, lote=frozenset({1}), rendered_by_id=rendered)
    assert isinstance(item, TransactionItem)
...
def test_validate_item_evidence_hit_normalized() -> None:
    rendered = {1: "Ana:  pagué   los $4.500 de la LUZ"}
    item = validate_item(
        TransactionItem, dict(_VALID), lote=frozenset({1}), rendered_by_id=rendered
    )
    assert isinstance(item, TransactionItem)

...
def test_get_cursor_returns_none_when_unset(conn: Any, seed_source: dict) -> None:
    assert checkpoint.get_cursor(conn, seed_source["id"]) is None


def test_save_and_get_roundtrip(conn: Any, seed_source: dict) -> None:
...
def test_save_overwrites_existing(conn: Any, seed_source: dict) -> None:
...
def test_checkpoints_isolated_by_source(conn: Any, seed_source: dict) -> None:
    src2 = conn.execute(
        text(

...
(`chat_id`/`chat_title`/`chat_kind`), full-sweep idempotente, con el título más reciente (los
grupos se renombran). La llama `build_relations` antes de la provenance.
...
def sync_canales(conn: Connection, user_id: int) -> int:
    """Upsert de los canales de CHAT del user desde inbox (idempotente; actualiza
    `display_name`/`chat_kind` al valor más reciente). Devuelve cuántos canales viven."""
    conn.execute(
        text(
            """
